...
export default function Search() {
  ...
  const handleFilter = (values: { selectCategory: string; selectAlcoholic: string; selectGlass: string }) => {
    ...
    if(values.selectAlcoholic != '') {
      if(filteredDrinkData.length = 0) {
        const drinksFilteredByA = drinkData.filter((drink: any) => drink.strA === values.selectA);
        setFilteredDrinkData(drinksFilteredByA)
      } else {
        const drinksFilteredByA = filteredDrinkData.filter((drink: any) => drink.strA === values.selectA);
        setFilteredDrinkData(drinksFilteredByA)
      }
    }

...
export default function Search() {
  ...
  return (
    <main className="w-full h-screen px-4 py-2 md:px-8 md:py-6 bg-gradient-to-r from-[#251A14] to-[#120D0A] overflow-auto">
      <HeaderSearch />
      <h2 className="text-white text-xl mt-36">RELATED DRINKS</h2>
      <div className="flex">
        <div className="flex flex-col justify-start gap-8">
          <FilterForm onChange={handleFilter} />
          {filteredDrinkData && <button onClick={handleCancelFilter} className="bg-red-700 py-3 px-6 max-w-fit rounded-md text-white font-sans">Cancel filter</button>}
        </div>
        <div className="w-[1px] h-[40rem] flex-shrink-0 bg-black ml-20"/>
        {error ? <span className="text-2xl text-red-700 underline ml-28">Sorry 😔, an error occurred in the catalog. Try again with another drink or inform us.</span> :
          <div className="grid grid-cols-4 gap-10 ml-28">
            {!paginateDrinkData ? <span className="text-2xl text-gray-500 underline ml-5">Loading...</span> : 
            !paginateDrinkData[0].idDrink ? <span className="text-2xl text-red-700 underline ml-5">Sorry 😔, we do not have this drink in the catalog</span> :

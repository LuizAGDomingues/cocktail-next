...
export default function Search() {
  ...
  const [paginateDrinkData, setPaginateDrinkData] = useState<Drink[] | null>(null)
    ...
    <main className="w-full h-screen px-4 py-2 md:px-8 md:py-6 bg-gradient-to-r from-[#251A14] to-[#120D0A] overflow-auto">
      ...
      <div className="md:flex">
        ...
        {error ? <span className="text-xl md:text-2xl text-red-700 underline ml-28">Sorry 😔, an error occurred in the catalog. Try again with another drink or inform us.</span> :
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 ml-12 md:ml-28">
            {paginateDrinkData===null ? <span className="text-xl md:text-2xl text-gray-500 underline md:ml-5">Loading...</span> : 
            !paginateDrinkData===null ? <span className="text-xl md:text-2xl text-red-700 underline md:ml-5">Sorry 😔, we do not have this drink in the catalog</span> :
      ...
      <div className="mt-12 flex items-center justify-center gap-20">
        {page > 1 && <button onClick={previousPage} className='inline-flex py-2 px-4 md:py-3 md:px-6 items-center justify-center rounded-lg text-white font-sans text-base bg-[#675045] hover:bg-[#443027] transition-colors duration-200'>Previous Page</button>}
        {(paginateDrinkData && paginateDrinkData.length === 8) && <button onClick={nextPage} className='inline-flex py-2 px-4 md:py-3 md:px-6 items-center justify-center rounded-lg text-white font-sans text-base bg-[#675045] hover:bg-[#443027] transition-colors duration-200'>Next Page</button>} 
      </div>
    </main>
  )
}
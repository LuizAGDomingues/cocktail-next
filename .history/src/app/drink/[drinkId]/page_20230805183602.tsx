...
export default function Drink({ params }: { params: { drinkId: string } }) {
  ...
  return (
    <main className="w-full h-screen px-8 py-6 bg-gradient-to-r from-[#251A14] to-[#120D0A] overflow-auto">
      ...
      <div className="mt-36 lg:ml-44 text-white">
        <div className="lg:flex items-start justify-center gap-14">
          <div className="flex gap-4 lg:flex-col lg:gap-0">
            <div className="relative">
              ...
              <button onClick={handleFavorite} disabled={!hasCookieName} className="absolute top-1 right-2 text-red disabled:text-gray-600">
                <HeartStraight size={32} weight={isFavorite ? "fill" : "regular"} />
              </button>
            </div>
            <div className="flex flex-col">
              <span className="text-xl lg:mt-5">{drinkData.strGlass}</span>
              <span className="text-xl mt-5 lg:mt-1">{drinkData.strCategory}</span>
              <span className="text-xl mt-5 lg:mt-1">{drinkData.strAlcoholic}</span>
            </div>
          </div>
          <div className="mt-5 lg:mt-0">
            <h3 className="text-2xl lg:text-4xl underline">Ingredients:</h3>
            <ul className="font-sans text-lg lg:text-xl mt-3">
            ...
            <h3 className="text-2xl lg:text-4x underline mt-14">Instructions:</h3>
            <p className="mt-3 w-[35rem] text-justify font-sans text-lg lg:text-xl mr-24">{drinkData.strInstructions}</p>
          </div>
        </div>
      </div>
    </main>

...
export default function Drink({ params }: { params: { drinkId: string } }) {
  ...
  const handleFavorite = () => {
    const hasFavorite = cookies().has('favorites');
    const favoriteDrinkData: {strDrinkThumb: string, strDrink: string, drinkId: string} = { strDrink: drinkData!.strDrink, strDrinkThumb: drinkData!.strDrinkThumb, drinkData!.}
    const favoriteDrinkDataJSON = JSON.stringify(favoriteDrinkData)
    if(!hasFavorite){
      'use server';
		  cookies().set('favorites', favoriteDrinkDataJSON);
    } else {
  ...
  return (
    <main className="w-full h-screen px-8 py-6 bg-gradient-to-r from-[#251A14] to-[#120D0A] overflow-auto">
      <HeaderSearch drinkName={drinkData.idDrink} />

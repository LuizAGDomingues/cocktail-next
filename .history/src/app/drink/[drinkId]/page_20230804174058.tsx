...
interface favoriteDateProps {
  {strDrinkThumb: string, strDrink: string, drinkId: string}
}
...
export default function Drink({ params }: { params: { drinkId: string } }) {
  ...
  useEffect(() => {
    ...
    const filter = cookiesFavoriteData.filter((favorite: {strDrinkThumb: string, strDrink: string, drinkId: string}) => {})
  }, [])

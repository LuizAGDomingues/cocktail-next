...
import { googleConnectUser } from '@/lib/firebase/login'
...
function LoginDialog() {
  ...
  const handleGoogleLogin = async () => {
    ...
    if(Array.isArray(googleLogin)) {
      ...
      'use server';
		  cookies().set('loginToken', googleLogin);
      setLoginSucess(true)
    }
  }
      ...
      <DialogContent>
        ...
        <form onSubmit={handleLogin_Create} className='mt-16 flex flex-col justify-start w-full gap-6'>
          <div>
            ...
            <input type="text" name="email" id="email" value onChange={(e) => setEmail(e.target.value)} className='w-full py-2 px-2 flex items-center rounded-lg font-sans bg-[#E4E4E4] text-black placeholder:text-[#646464]' placeholder='example@email.com' />
          </div>
          <div>
            <label htmlFor="password" className='text-white text-center font-sans'>{signIn ? 'Enter your Password' : 'Create your password'}</label>
            <input type="password" name="password" id="password" onChange={(e) => setPassword(e.target.value)} className='w-full py-2 px-2 flex items-center rounded-lg font-sans bg-[#E4E4E4] text-black placeholder:text-[#646464]' placeholder='********' />
          </div>
          {!signIn &&
            <div>
              <label htmlFor="password" className='text-white text-center font-sans'>Confirm your password</label>
              <input type="password" name="password" id="password" onChange={(e) => setCreatePassword(e.target.value)} className='w-full py-2 px-2 flex items-center rounded-lg font-sans bg-[#E4E4E4] text-black placeholder:text-[#646464]' placeholder='********' />
            </div>
          }
          {signIn 

...
function LoginDialog() {
  ...
  const handleConnectLogin = async () => {
    const connectLogin = await connectUser()

    if(Array.isArray(connectLogin)) {
      setLoginError(true)
      console.log(connectLogin[0], connectLogin[2])
    } else {
      'use server';
		  cookies().set('loginToken', connectLogin);
      setLoginSucess(true)
    }
  }
  ...
  const handleCreateLogin = async () => {
    ...
    if(Array.isArray(googleLogin)) {
      setLoginError(true)
      console.log(googleLogin[0], googleLogin[2])
    } else {
      'use server';
    ...
    <Dialog>
      ...
      <DialogContent>
        ...
        <form className='mt-16 flex flex-col justify-start w-full gap-6'>
          ...
          {signIn 
          ? <button onClick={handleConnectLogin} className='mt-3 uppercase max-w-fit mx-auto font-sans text-white rounded-lg px-6 py-3 bg-[#5F4B43] hover:bg-[#422f28]'>Enter</button>
          : <button onClick={handleCreateLogin} className='mt-3 uppercase max-w-fit mx-auto font-sans text-white rounded-lg px-6 py-3 bg-[#5F4B43] hover:bg-[#422f28]'>Create account</button>
          }
        </form>

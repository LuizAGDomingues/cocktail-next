...
function LoginDialog({onUserLoged}: LoginDialogProps) {
  ...
  return (
    <Dialog>
      <DialogTrigger>
        <button className='inline-flex px-6 py-3 items-center justify-center rounded-lg text-white font-sans bg-[#675045] hover:bg-[#443027] transition-colors duration-200'>LOGIN</button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            <h2 className='text-white font-sans text-center text-xl md:text-3xl'>Welcome</h2>
            <span className='text-white font-sans text-center md:text-lg'>Please, enter your details</span>
          </DialogTitle>
        </DialogHeader>
        <div className='inline-flex p-1 w-64 justify-center items-center bg-[#A3A1A1] rounded-lg font-sans'>
        ...
        <form onSubmit={handleLogin_Create} className='mt-16 flex flex-col justify-start w-full gap-6'>
          <div>
            ...
            <input type="text" name="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} className='md:w-full py-2 px-2 flex items-center rounded-lg font-sans bg-[#E4E4E4] text-black placeholder:text-[#646464]' placeholder='example@email.com' />
          </div>
          <div>
            <label htmlFor="password" className='text-white text-center font-sans'>{signIn ? 'Enter your Password' : 'Create your password'}</label>
            <input type="password" name="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} className='md:w-full py-2 px-2 flex items-center rounded-lg font-sans bg-[#E4E4E4] text-black placeholder:text-[#646464]' placeholder='********' />
          </div>
          {!signIn &&
            <div>
              <label htmlFor="password" className='text-white text-center font-sans'>Confirm your password</label>
              <input type="password" name="password" id="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className='md:w-full py-2 px-2 flex items-center rounded-lg font-sans bg-[#E4E4E4] text-black placeholder:text-[#646464]' placeholder='********' />
              {passwordError && <span className="text-red-600 mt-1">The passwords must be equal</span>}
            </div>
          }
          {signIn 
          ...
          {loginError && <span className="text-red-600 mt-1">An error happened. Please, check your informations.</span>}
        </form>
        <div className='flex items-center justify-center gap-2 mt-20'>
          <div className='flex-shrink-0 h-[1px] md:w-40 bg-gradient-to-r from-[#1F1007] to-[#C2C2C2]'></div>
          <span className='text-[#C2C2C2] font-sans text-xs'>Or Connect With</span>
          <div className='flex-shrink-0 h-[1px] md:w-40 bg-gradient-to-r from-[#C2C2C2] to-[#1F1007]'></div>
        </div>
